...
export class IdentityProof extends Struct {
    ...
    /**
     * Create a new instance from an PICOIO authorization header string.
     * "PICOIO <base64payload>"
     */
    static fromString(string: string) {
        const parts = string.split(' ')
        if (parts.length !== 2 || parts[0] !== 'PICOIO') {
    ...
    /**
     * Encode the proof to an `PICOIO` auth header string.
     */
    toString() {
        const data = Serializer.encode({object: this})
        return `PICOIO ${Base64u.encode(data.array, false)}`
    }
}
